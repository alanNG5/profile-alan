...
export class SalesService {
  ...
  async getDeliveryStatusStats() {
    try {
      ...
      let setNextDate = new Date(
        new Date().getFullYear(),
        new Date().getMonth(),
        new Date().getDate() + 1
      );

      let tmr = new Date(setNextDate).getDate();
      let tmrStr = ("0" + tmr).slice(-2);
      let monthTmr = new Date(setNextDate).getMonth() + 1;
      let monthTmrStr = ("0" + monthTmr).slice(-2);
      let yrTmr = new Date(setNextDate).getFullYear();
      ...
      let data = await this.knex.raw(
        ...
        FROM sales
        WHERE created_at >= '${yearArr[0]}-${recentMonthArr[0]}-01' AND created_at < '${yrTmr}-${monthTmrStr}-${tmrStr}'
        GROUP BY order_status)
        SELECT * FROM Subtotal
        UNION ALL
        SELECT 'monthly_count' AS order_status,
        SUM("two_months_ago") AS "twoMonthsAgo",
